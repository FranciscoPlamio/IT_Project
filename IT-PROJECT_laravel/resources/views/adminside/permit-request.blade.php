<x-admin-layout :title="'Request History'">
                                ...
                                <tr
                                    ...
                                    <td>
                                        @php
                                            ...
                                            if (strtolower($req->form_type) === 'form1-09') {
                                                try {
                                                    $files = Storage::disk('local')->files("forms/{$req->form_token}");
                                                    foreach ($files as $file) {
                                                        if (Str::startsWith(basename($file), 'permit_')) {
        ...
        <div class="certificate-modal__content">
            ...
            <div style="margin-bottom:20px;">
                <div style="margin-bottom:12px;">
                    <label style="display:block;font-weight:600;margin-bottom:4px;font-size:14px;">Applicant
                        Name:</label>
                ...
                <div style="margin-bottom:12px;">
                    <label style="display:block;font-weight:600;margin-bottom:4px;font-size:14px;">Permit Type:</label>
                    <span id="cert_type" style="display:block;padding:8px;background:#f1f5f9;border-radius:6px;"></span>
                </div>
                <div style="margin-bottom:12px;">
                    <label style="display:block;font-weight:600;margin-bottom:4px;font-size:14px;">Radio
                        Service:</label>
                    <span id="cert_radio_service"
                        style="display:block;padding:8px;background:#f1f5f9;border-radius:6px;"></span>
                </div>
                <div style="margin-bottom:12px;">
                    <label style="display:block;font-weight:600;margin-bottom:4px;font-size:14px;">Application
                        Type:</label>
                    <span id="cert_app_type"
                        style="display:block;padding:8px;background:#f1f5f9;border-radius:6px;"></span>
                </div>
                <div style="margin-bottom:12px;">
                    <label style="display:block;font-weight:600;margin-bottom:4px;font-size:14px;">Intended Use:</label>
                    <span id="cert_int_use"
                        style="display:block;padding:8px;background:#f1f5f9;border-radius:6px;"></span>
                </div>
                <div style="margin-bottom:12px;">
    ...
    <script>
        ...
        function openCertificateModal(token) {
            ...
            console.log(`/admin/get-permit-data/${token}`);

            // Fetch form data
            fetch(`/admin/get-permit-data/${token}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('cert_last_name').textContent = data.applicant || '—';
                    ...
                    document.getElementById('cert_radio_service').textContent = data.radio_service || '—';
                    document.getElementById('cert_app_type').textContent = data.application_type || '—';
                    document.getElementById('cert_int_use').textContent = data.intended_use || '—';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('previewCertificate').addEventListener('click', () => {
                if (currentCertificateToken) {
                    window.open(`/admin/generate-permit?token=${currentCertificateToken}&preview=1`,
                        '_blank');
                }
            });
            ...
            document.getElementById('generateCertificate').addEventListener('click', () => {
                if (currentCertificateToken) {
                    window.location.href =
                        `/admin/generate-permit?token=${currentCertificateToken}&preview=0`;
