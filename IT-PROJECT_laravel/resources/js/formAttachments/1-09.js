export default function attachmentsForm102(containerId, form) {
    ...
    const intendedUse = form.intended_use;
    console.log(examType, applicationType, intendedUse);
    ...
    //AMATEUR
    if (examType.includes("at-rsl")) {
        if (intendedUse.includes("new")) {
            labels = {
                photocopy_of_atrsl_or_report_of_rating:
                    "Photocopy of valid AT-ROC or Report of Rating",
            };
        } else if (
            intendedUse.includes("change") ||
            intendedUse.includes("additional")
        ) {
            labels = {
                photocopy_of_atrsl: "Photocopy of valid AT-RSL",
            };
        }
    } else if (examType === "at-lifetime") {
        labels = {
            photocopy_of_supplementary_certificate:
                "Photocopy of Supplementary Certificate",
        };
    } else if (examType === "at-club-rsl") {
        labels = {
            photocopy_of_supplementary_certificate:
                "Photocopy of Supplementary Certificate",
            list_of_licensed_amateur_club_members:
                "List of licensed Amateur Club Trustee, Officers, and Members",
            photocopy_of_memorandum_of_agreement_with_ntc:
                "Photocopy of the Memorandum of Agreement with NTC",
            station_location_map:
                "Map showing the station location with geographical coordinates",
        };
    } else if (examType === "storage-permit") {
        labels = {
            photocopy_of_atrsl: "Photocopy of valid AT-RSL",
        };
    }
