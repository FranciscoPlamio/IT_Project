...
export default function initSharedValidator() {
    ...
    try { console.debug('sharedValidator module loaded'); } catch (e) {}

    document.querySelectorAll('form').forEach((form) => {
    
    try { console.debug('sharedValidator checking form', form.id || form.getAttribute('action') || 'unnamed'); } catch (e) {}
        
    if (form.id === 'form101') return;
        try { console.debug('sharedValidator initializing for form', form.id || form.getAttribute('action') || 'unnamed'); } catch (e) {}
        ...
        try {
            ...
            if (raw) {
                ...
                console.debug('sharedValidator loaded form-level rules for', form.id || form.getAttribute('action') || 'unnamed');
            }
        } catch (e) {
            console.warn('sharedValidator: failed to parse data-validation-rules for form', form.id, e);
            formLevelRules = {};
        }
        ...
        function validateField(field) {
            ...
            const dataValidation = (field.getAttribute('data-validation') || '').toString();
            const value = (field.value || '').toString().trim();

            // determine label-required marker
            const labelEl = field.parentNode ? field.parentNode.querySelector('label') : null;
            const labelText = labelEl ? (labelEl.textContent || '') : '';
            const labelRequired = (labelText && labelText.indexOf('*') !== -1) || Boolean(field.closest('.form-field')?.querySelector('.required-asterisk'));
            ...
            const perForm = (name && formLevelRules[name]) ? formLevelRules[name] : {};

            // infer rules from type/name/attributes when explicit rules absent
            const inferred = {};
            const lname = name.toLowerCase();
            if (field.type === 'email' || lname.includes('email')) inferred.type = 'email';
            if (field.type === 'date' || lname.includes('dob') || lname.includes('date')) inferred.type = 'date';
            if (field.type === 'tel' || lname.includes('phone') || lname.includes('contact')) inferred.pattern = /^(\+63|0)?[0-9]{10}$/;
            if (lname.includes('zip') || lname.includes('zipcode')) inferred.pattern = /^\d{4}$/;
            if (lname.includes('year')) inferred.pattern = /^\d{4}$/;
            if (lname.includes('name') || lname.includes('first') || lname.includes('last')) inferred.pattern = /^[A-Za-z\s]+$/;
            if (field.tagName && field.tagName.toLowerCase() === 'select') inferred.type = 'select';

            // determine required: explicit attribute, per rule, or label marker
            const isRequired = Boolean(field.required) || Boolean(baseRule.required) || Boolean(perForm.required) || labelRequired;
            ...
            if (rule.type === 'email' && value) {
                ...
                if (!emailPattern.test(value)) {
                    showFieldValidation(field, `Please enter a valid email address.`);
                    return false;
                }
            }
            ...
            if (rule.pattern && value) {
                ...
                if (!pattern.test(value)) {
                    ...
                    if (dataValidation === 'phone' || /phone|contact/.test(name)) msg = 'Please enter a valid Philippine phone number (e.g., 09123456789).';
                    if (dataValidation === 'year' || /year/.test(name)) msg = 'Please enter a valid 4-digit year (e.g., 2020).';
                    if (dataValidation === 'zipcode' || dataValidation === 'zip_code' || /zip/.test(name)) msg = 'Please enter a valid 4-digit ZIP code (e.g., 1000).';
        ...
        function validateForm(e) {
            ...
            groups.forEach(group => {
                ...
                const any = Array.from(items).some(el => {
                    ...
                    return Boolean(el.value && el.value.toString().trim() !== '');
                });
                if (!any) {
                    invalid.push(group);
                }
            ...
            fields.forEach(field => {
                ...
                if ((field.type === 'radio' || field.type === 'checkbox') && field.name) {
                    ...
                    if (group && group.length > 0 && group[0] === field) {
                        ...
                        if ((isRequired || field.hasAttribute('data-validation')) && !anyChecked) {
            ...
            if (invalid.length > 0) {
                ...
                summaryDiv.style.cssText = 'background-color:#f8d7da;border:2px solid #dc3545;border-radius:6px;padding:12px;margin:12px 0;color:#dc3545;font-weight:600;';
                const labels = invalid.slice(0, 10).map(f => {
                    if (f.getAttribute && f.getAttribute('data-require-one')) return 'Required selection';
                    return getFieldLabel(f instanceof Element && f.closest ? (f.closest('.form-field')?.querySelector('label') || f) : f) || 'Required field';
                });
                ...
                const firstInvalid = invalid[0] instanceof Element ? invalid[0] : form.querySelector('input.field-error, select.field-error, textarea.field-error');
        ...
        function validateSection(section) {
            ...
            if (invalid.length > 0) {
                ...
                if (section.firstChild) section.insertBefore(summaryDiv, section.firstChild);
        ...
        nextButtons.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const activeStep = form.querySelector('.step-content.active') || btn.closest('.step-content') || form;
                try { console.debug('sharedValidator next button clicked, validating section', activeStep); } catch (err) {}
